package me.risky.jlike.service;
...
public class WelfareService {
	...
	public List<News> getWelfareList(String htmlContent, int type){
		List<News> list = new ArrayList<News>();
		try {
			Document doc = Jsoup.parse(htmlContent);
			Elements error = doc.getElementsByClass("error404");
			if(error.size() > 0){
				Log.d("error", error.size()+"");
				return list;
			}
